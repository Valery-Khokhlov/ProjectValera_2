...
+ Модуль test_generators.py
  Содержит тесты для обработки функций filter_by_currency, transaction_descriptions, card_number_generator
  Применён  декоратор @pytest.mark.parametrize  
...
**test_transaction_descriptions(transactions_: List[Dict[str, Any]], expected_descriptions: List[str]):**
...
**test_card_number_generator(start: int, stop: int, expected: List[str]):**

Тест проверяет, что генератор выдает правильные номера карт в заданном диапазоне,
проверяет корректность форматирования номеров карт, корректно обрабатывает крайние 
